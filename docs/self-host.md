...
appears on sign-in only when a transactional email provider is available. OtterBot uses a
...
EMAIL_FROM=OtterBot <no-reply@example.com>
```
...
STARTTLS. OtterBot rejects configuration that disables TLS or certificate verification.
...
advanced settings; mobile inherits the same backend policy. OtterBot sends standard
...
- **E2B** runs bot computers away from the OtterBot host and is a good choice for public or multi-user
  production deployments. OtterBot checkpoints the portable workspace and browser-profile directory to
...
- **Box by ASCII** provides a managed Linux desktop through `BOX_API_KEY` and optionally
  `BOX_API_URL`. OtterBot always creates or resumes boxes with `noEnv: true`, keeps the portable
