...
describe("server address normalization", () => {
  ...
  it("rejects cleartext link-local addresses used by cloud metadata endpoints", () => {
    ...
    // HTTPS to link-local still normalizes; the health probe must match OtterBot.
  ...
  it.each(["", "   ", "not a url", "ftp://example.com", "file:///etc/passwd", "http://"])(
    "rejects an address that cannot reach an OtterBot server (%s)",
    (value) => {
...
describe("OtterBot health response", () => {
