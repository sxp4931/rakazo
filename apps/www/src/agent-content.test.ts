...
describe("agent content negotiation", () => {
  ...
  it("maps canonical and trailing-slash page paths to Markdown documents", () => {
    expect(getMarkdownDocument("/")).toContain("# OtterBot");
    expect(getMarkdownDocument("/about/")).toContain("# About OtterBot");
  ...
  it("publishes specific when-to-use instructions for agents", () => {
    ...
    expect(AGENT_INSTRUCTIONS).toContain("## When to use OtterBot");
    expect(AGENT_INSTRUCTIONS).toContain("## How an agent should use OtterBot");
  ...
  it("returns cache-safe Markdown responses and omits bodies for HEAD", async () => {
    const response = markdownResponse("# OtterBot\n");
    ...
    await expect(response.text()).resolves.toBe("# OtterBot\n");

    const headResponse = markdownResponse("# OtterBot\n", "HEAD", 404);
