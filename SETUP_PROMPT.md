# Set up OtterBot with a coding agent
...
```text
Set up OtterBot from published GHCR images and leave the web UI running.
...
1. Which directory should contain the OtterBot folder (or use the current directory)?
2. How should models be connected?
   - Add a deployment-wide `OPENROUTER_API_KEY` to `.env`.
   - Connect during OtterBot onboarding with a provider API key or with ChatGPT Plus/Pro, GitHub Copilot, or SuperGrok / X Premium.
...
```text
Set up OtterBot locally and leave it running in a usable state.
...
1. Should you clone into the current directory, or what parent directory should contain `rakazo`? If you are already inside an OtterBot checkout, offer to use it without recloning.
2. How should models be connected?
   - Add a deployment-wide `OPENROUTER_API_KEY` to `.env`.
   - Connect during OtterBot onboarding with a provider API key or with ChatGPT Plus/Pro, GitHub Copilot, or SuperGrok / X Premium.
   - Defer model setup and verify infrastructure only. Make clear that bots cannot answer until a model is connected.
