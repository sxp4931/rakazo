...
[[ "$offline_out" == *"OtterBot is starting"* ]] || fail "--offline did not start"
...
[[ "$old_out" != *"OtterBot setup failed:"* ]] || fail "old Compose --offline should not hard-fail: $old_out"
[[ "$old_out" == *"OtterBot is starting"* ]] || fail "old Compose --offline should continue: $old_out"
